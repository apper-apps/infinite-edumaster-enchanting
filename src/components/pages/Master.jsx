...
const Master = () => {
  ...
  const loadVideos = async () => {
    try {
      ...
      const masterVideos = data.filter(video => video.category === "master");
      
      // Sort: pinned first, then by creation date
      const sortedVideos = masterVideos.sort((a, b) => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
        <div className="mb-4 md:mb-0">
          ...
          <p className="text-gray-600">마스터 등급 전용 프리미엄 동영상 강의</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Button 
            ...
            className="flex items-center gap-2 whitespace-nowrap bg-gradient-to-r from-secondary-500 to-accent-500 hover:from-secondary-600 hover:to-accent-600"
          >
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg">
          <div className="flex items-center">
            <div className="p-3 bg-secondary-100 rounded-lg">
              <ApperIcon name="Star" className="text-secondary-600" size={24} />
            </div>
            <div className="ml-4">
              <p className="text-2xl font-bold text-gray-900">{videos.length}</p>
              <p className="text-gray-500">마스터 강의</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg">
          <div className="flex items-center">
            <div className="p-3 bg-primary-100 rounded-lg">
              <ApperIcon name="PlayCircle" className="text-primary-600" size={24} />
            </div>
            <div className="ml-4">
              <p className="text-2xl font-bold text-gray-900">
      ...
      {filteredVideos.length === 0 ? (
        <Empty
          title="마스터 강의가 없습니다"
          description="아직 등록된 마스터 강의가 없습니다. 첫 번째 프리미엄 강의를 업로드해보세요."
          icon="Star"
          actionLabel="동영상 업로드"
